...
class sharepoint {
  ...
  public function __construct(){
    ...
    if(!empty($this->spListsWS) && !empty($this->spUser) && !empty($this->spPW)){
      ...
      die('SharePoint credentials are undefined.');
    }
  ...
  function buildQuery(){
    ...
    if($this->spService == 'GetListItems'){
      ...
      if(!empty($this->camlViewFields) && is_array($this->camlViewFields) && count($this->camlViewFields) > 0){
        ...
        // retrieve all fields
        $this->camlQuery.= "<viewFields><ViewFields/></viewFields>";
      }
    }
    elseif(!empty($this->camlUpdate)){
  ...
  function executeQuery(){
    ...
    if(!empty($clientError)){
      ...
      if(isset($return['GetListItemsResult']['listitems']['data']['row'])){
        ...
        if(!empty($this->camlViewFields) && is_array($this->camlViewFields) && count($this->camlViewFields) > 0){
          // scrub data before saving (dramatically reduces memory use)
          $filterFields = array();
          ...
          // sharepoint returns either an associative array or a multidimensional associative array based on the number of results
  ...
  function downloadFile($sourceFile=null, $downloadDestination=null){ // download file from SharePoint to specified destination
    ...
    if(!empty($sourceFile) && !empty($downloadDestination)){
      ...
      die('File and/or destination are missing.');
    }
  }
  ...
  function downloadLibrary($listName=null, $listDirectory=null, $downloadDestination=null){
    /*if(!empty($downloadDestination)){
      ...
      if(is_dir($downloadDestination) && substr(decoct(fileperms($downloadDestination)),2) != '775'){
        ...
        $this->logError("Destination directory has insufficient permissions to perform caching.");
      }
    }*/
  ...
  function extractDocuments($source){
    ...
    if(preg_match_all('/<a\s[^>]*href=[\"\']([^\"\']('.preg_quote($spSiteRoot, '/').'|[^>]*'.preg_quote(str_ireplace($spRoot, '', $spSiteRoot), '/').'[^>]*)*\.(doc|docx|pdf|pps|ppt|pptx|csv|xls|xlsx|zip))[\"\'][^>]*>?/i', $source, $match)) // extract document paths from source // this method supports files from other domains
      return $match[1];
    else
      return false;
  }

  // to make it easier to report errors using a custom function if desired
  function logError($errorMsg=NULL){
    error_log($errorMsg);
  }
